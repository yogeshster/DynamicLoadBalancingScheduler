...
//	printf("\n%s\n",address);
	...
	pthread_create(&periodicCheckpointThread,NULL,periodicCheckpoint,NULL);
	pthread_create(&pth1,NULL,threadfunc,NULL);

	while(1)
	{
		//gets(buff);
		//write(sockdes,buff,sizeof(buff));
		//write(sockdes,buff1,sizeof(buff));
		//sleep(3);
		...
		for(i=len-2,j=3;j>=0;j--,i--)
		{
			load15[j]=str[i];
		}
		load15[4]='\0';		//load -two decimal places
		//printf("Load15: %s ",load15);
		for(i=len-8,j=3;j>=0;j--,i--)
		{
			load5[j]=str[i];
		}
		load5[4]='\0';		//load -two decimal places
		//printf("Load 5: %s ",load5);
		for(i=len-14,j=3;j>=0;j--,i--)
		{
			load1[j]=str[i];
		}
		load1[4]='\0';		//load -two decimal places
		//printf("Load 1: %s ",load1);
		
		strcpy(buff1,load1);strcat(buff1," ");strcat(buff1,load5);strcat(buff1," ");strcat(buff1,load15);strcat(buff1,"|");strcat(buff1,address);
		write(sockdes,buff1,sizeof(buff1));

		while(OK==0){}
		write(sockdes,message,sizeof(message));
